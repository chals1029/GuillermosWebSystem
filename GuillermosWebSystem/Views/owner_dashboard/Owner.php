...
<head>
  ...
  <style>
    ...
    .sidebar{background:#fff;height:100vh;padding-top:20px;position:fixed;width:240px;text-align:center;box-shadow:2px 0 10px rgba(0,0,0,.05);}
    .sidebar h4{font-weight:700;color:#4d2e00;margin-bottom:20px;font-size:1.5rem;}
    .sidebar-item{display:block;padding:15px 25px;color:#3b2c23;font-weight:600;cursor:pointer;transition:all .3s;border-radius:8px;margin:5px 15px;}
    .sidebar-item.active,.sidebar-item:hover{background:#e2c9a7;color:#4d2e00;}
    .header{background:#6B4F3F;color:#fff;padding:20px 40px;position:fixed;left:240px;right:0;top:0;z-index:10;box-shadow:0 2px 10px rgba(0,0,0,.1);display:flex;justify-content:space-between;align-items:center;}
    .header .title p{margin:0;font-size:1rem;}
    .main{margin-left:240px;margin-top:100px;padding:20px 40px;}
...
<body>
  ...
  <div class="sidebar">
    ...
    <div class="sidebar-item" data-page="funding">Funding Projections</div>
    <div class="sidebar-item" data-page="performance">Product Performance</div>
    <div class="sidebar-item" data-page="inventory">Inventory</div>
  </div>
        ...
        <li>
          <div class="user-info d-flex align-items-center gap-3">
            ...
            <div class="user-details">
              <h6>Guillermo Owner</h6>
              <p>owner@guillermos.com</p>
            </div>
          </div>
        </li>
  ...
  <div class="main" id="page-content">
    ...
    <!-- ==================== FUNDING PROJECTIONS ==================== -->
    <div class="page" id="funding" style="display:none;">
    ...
    <!-- ==================== PRODUCT PERFORMANCE (FULLY RESTORED) ==================== -->
    <div class="page" id="performance" style="display:none;">
      ...
      <div class="perf-header">
        ...
        <div class="dropdown">
          ...
          <ul class="dropdown-menu">
            ...
            <li><a class="dropdown-item" href="#" data-cat="Fruits & Yogurt">Fruits & Yogurt</a></li>
    ...
    <div class="page" id="inventory" style="display:none;">
      <h3 class="mb-4" style="color:#4d2e00;font-weight:700;">Inventory</h3>
        ...
        <div class="modal-body">
          <form id="productForm">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" name="Low_Stock_Alert" id="lowStockSelect" required>
  ...
  <script>
    ...
    document.querySelector(`.sidebar-item[data-page="${currentPage}"]`)?.click();
    ...
    function updateDateTime() {
      ...
      document.getElementById('datetime').textContent = new Date().toLocaleString('en-PH', opts);
    }
    ...
    /* ---------- Product Performance Logic (FULLY RESTORED) ---------- */
    ...
    /* ---------- Inventory Logic (Database-backed) ---------- */
    ...
    function normalizeProduct(product) {
      ...
      const descriptionValue = product.Description ?? product.description ?? null;
  ...
  const subCategoryValue = product.Sub_category ?? product.sub_category ?? null;
  const subCategory = subCategoryValue === null ? null : (String(subCategoryValue).trim() || null);
      ...
      const lowValue = (product.Low_Stock_Alert ?? product.low_stock_alert ?? 'Safe').toString().trim();
      const lowAlert = lowValue === '' ? 'Safe' : lowValue;
    ...
    function renderInventory(filter = '') {
      ...
      const rows = filtered.map(p => {
        ...
        const stock = Number.isFinite(p.Stock_Quantity) ? p.Stock_Quantity : Number.parseInt(p.Stock_Quantity, 10);
        const lowAlert = (p.Low_Stock_Alert || 'Safe').toString();
        const lowClass = ['low', 'critical'].includes(lowAlert.toLowerCase()) ? 'stock-low' : '';
        ...
        <tr data-id="${p.Product_ID}">
          ...
          <td>${Number.isNaN(stock) ? '—' : stock}</td>
          <td class="${lowClass}">${lowAlert}</td>
          <td>
    ...
    function resetFormDefaults() {
      productForm.reset();
      productForm.Low_Stock_Alert.value = 'Safe';
    ...
    inventoryTable.addEventListener('click', async e => {
      ...
      if (editBtn) {
        ...
        productForm.Low_Stock_Alert.value = product.Low_Stock_Alert || 'Safe';
        modal.show();
        return;
      }
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      const productId = isUpdate ? Number.parseInt(payload.Product_ID, 10) : null;
      ...
      payload.Sub_category = (payload.Sub_category ?? '').trim();
      if (payload.Sub_category === '') {
        payload.Sub_category = null;
      }

  payload.Low_Stock_Alert = (payload.Low_Stock_Alert || 'Safe').trim();
