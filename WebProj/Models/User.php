...
class User
{
    ...
    {
        $sql = 'SELECT user_id, username, password, name, email, user_role FROM users WHERE username = ? OR email = ? LIMIT 1';
        $stmt = $this->conn->prepare($sql);
        if (!$stmt) return null;
    ...
    {
        $sql = 'SELECT user_id, username, password, name, email, user_role FROM users WHERE email = ? LIMIT 1';
        $stmt = $this->conn->prepare($sql);
        if (!$stmt) {
            return null;
        }
